...
const MultiStepForm = () => {
  ...
  const steps = [
    {
      ...
      icon: FaNetworkWired,
      component: (
        <Step1NetworkData formData={formData} setFormData={setFormData} />
      ),
    },
    {
      ...
      icon: FaShieldAlt,
      component: (
        <Step2ThreatAnalysis formData={formData} setFormData={setFormData} />
      ),
    },
    {
      label: "User & Device Info",
      icon: FaUser,
      component: (
        <Step3UserDeviceInfo formData={formData} setFormData={setFormData} />
      ),
    },
    {
      label: "Logs & Alerts",
      icon: FaFileAlt,
      component: (
        <Step4LogsAlerts formData={formData} setFormData={setFormData} />
      ),
    },
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axiosInstance.post("/data/submit", { data: formattedFormData });
      const { attack_type, severity_level } = response.data;
      console.log(
        "%cBackend Response:",
      ...
      setPrediction({ attackType: attack_type, severity: severity_level });
      onOpen();
    } catch (error) {
      toast({
  ...
  return (
    <Flex justify="center" align="center" minH="100vh" bg={bgColor} p={6}>
      ...
      >
          ...
          {steps.map((step, index) => (
            <Flex key={index} direction="column" align="center">
              <Icon
                as={step.icon}
                boxSize={6}
                color={currentStep === index + 1 ? "green.500" : "gray.400"}
              />
        ...
        <VStack spacing={6}>{steps[currentStep - 1].component}</VStack>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Text>
              <strong>Predicted Attack Type:</strong>{" "}
            ...
            <Text mt={2}>
              ...
              <Badge
                colorScheme={
                  prediction.severity === "Critical" ? "red" : "green"
                }
              >
                {prediction.severity}
              </Badge>
